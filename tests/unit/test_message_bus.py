...
@final
class ShortCircuit(MiddlewareInterface):
    def __init__(self, name: str, calls: list[str]) -> None:
        self._name = name
        self._calls = calls
...
@final
class Capturing(MiddlewareInterface):
...
@final
class Exploding(MiddlewareInterface):
    @override
...
async def test_a_raw_message_reaches_the_chain_wrapped_and_stampless() -> None:
    ...
    _ = await MessageBus([capture]).dispatch("payload")
...
async def test_dispatch_stamps_are_attached_before_the_chain_runs() -> None:
    ...
    _ = await MessageBus([capture]).dispatch("payload", Marker("up-front"))
...
async def test_an_existing_envelope_is_redispatched_with_its_stamps_intact() -> None:
    ...
    _ = await MessageBus([capture]).dispatch(envelope)
